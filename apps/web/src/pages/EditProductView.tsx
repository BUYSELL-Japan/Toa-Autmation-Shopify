...
export function EditProductView() {
    ...
    const handleTranslate = async () => {
        ...
        setTranslating(true);
        try {
            const res = await fetch(`${API_BASE_URL}/products/translate`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    title: product.title,
            ...
            // Convert Object {en: {...}} to Array [{language_code: 'en', ...}]
            const newTranslations = Object.entries(result).map(([lang, content]: any) => ({
                language_code: lang,
                title: content.title,
                body_html: content.body_html
            }));

            setProduct({ ...product, translations: newTranslations });
            alert('Translation completed! Please review and click Save.');
                        ...
                        <div className="w-full lg:w-auto shrink-0 space-y-4">
                            <div className="bg-[#1e293b]/50 p-4 rounded-xl border border-white/5 sticky top-4">
                                ...
                                <div className="flex flex-col gap-4">
                                    {images.length > 0 && (
                                        <div className="w-64 aspect-square bg-black/20 rounded-xl overflow-hidden border border-white/10 relative group">
